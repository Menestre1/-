...
Функция ЭтоОбъектНачальногоОбразаУзлаРИБ(Знач Объект) Экспорт
	...
	Возврат СтандартныеПодсистемыПовтИсп.ОбъектыНачальногоОбраза().Получить(Объект.ПолноеИмя()) <> Неопределено;
...
Процедура ПриПолученииОбъектовИсключенийПланаОбмена(Объекты, Знач РаспределеннаяИнформационнаяБаза) Экспорт
	...
	Объекты.Добавить(Метаданные.РегистрыСведений.СеансыВерсийРасширений);
...
Функция ДобавитьЭлементУсловногоОформления(Форма, Инструкция) Экспорт
	...
	Для Каждого Поле Из Поля Цикл
		...
		Если РежимОтладки И Форма.Элементы.Найти(ИмяЭлемента) = Неопределено Тогда
			...
			Сообщение.Текст = СтрШаблон(Шаблон, ИмяЭлемента);
...
Процедура СобратьСтатистикуКонфигурации() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДатыЗапретаИзменения") Тогда
		МодульВерсионированиеОбъектов = ОбщегоНазначения.ОбщийМодуль("ДатыЗапретаИзмененияСлужебный");
		МодульВерсионированиеОбъектов.СобратьСтатистикуКонфигурации();
	КонецЕсли;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСКонтрагентами") Тогда
		МодульВерсионированиеОбъектов = ОбщегоНазначения.ОбщийМодуль("ПроверкаКонтрагентов");
		МодульВерсионированиеОбъектов.СобратьСтатистикуКонфигурации();
	КонецЕсли;
...
Функция ДобавитьПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если Не ОбновлениеИнформационнойБазыСлужебный.ДобавитьПараметрыРаботыКлиентаПриЗапуске(Параметры) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ИменаСерверныхМодулей() Экспорт
	...
	Для Каждого ОбщийМодуль Из Метаданные.ОбщиеМодули Цикл
		...
		ЭтоСерверныйМодуль = ОбщийМодуль.Сервер;
		#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			ЭтоСерверныйМодуль = ЭтоСерверныйМодуль Или ИнформационнаяБазаФайловая;
		#КонецЕсли
		
		Если ЭтоСерверныйМодуль Тогда
...
Процедура ПередЗапускомПрограммы()
	...
	МинимальнаяВерсияПлатформы = "8.3.6.1848";
