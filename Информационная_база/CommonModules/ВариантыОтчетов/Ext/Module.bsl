	...
	Для Каждого ПолноеИмяВарианта Из КлючиВариантов Цикл
		...
		Пока Истина Цикл
			...
			Для Номер = 1 По Количество Цикл
				...
				Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ТипыНастроек, Тип, СбрасыватьПрочие) = Истина Тогда
...
//
Функция ПодключитьОтчетОбъект(СсылкаВариантаИлиОтчета, ПолучатьМетаданные) Экспорт
	Результат = Новый Структура("Объект, Имя, ПолноеИмя, Метаданные, Ссылка, ТекстОшибки");
	Результат.Вставить("Успех", Ложь);
	
	Если СсылкаВариантаИлиОтчета = Неопределено Тогда
		Результат.ТекстОшибки = СтрШаблон(
			НСтр("ru = 'В методе ""%1"" не указан параметр ""%2"".'"),
			"ПодключитьОтчетОбъект",
			"СсылкаВариантаИлиОтчета");
		Возврат Результат;
	ИначеЕсли ТипЗнч(СсылкаВариантаИлиОтчета) = Тип("СправочникСсылка.ВариантыОтчетов") Тогда
	...
	Для Каждого ФункциональнаяОпцияМетаданные Из Метаданные.ФункциональныеОпции Цикл
		Если ФункциональнаяОпцияМетаданные.Хранение = КонстантаМетаданные Тогда
			...
			Если ТаблицаОпций.Найти(ФункциональнаяОпцияМетаданные.Имя, "ИмяФункциональнойОпции") <> Неопределено Тогда
				СтандартныеПодсистемыКлиентСервер.ОповеститьОткрытыеФормы(Результат, ИмяСобытия);
				Возврат;
			КонецЕсли;
...
Функция ПараметрыОткрытия(ВариантСсылка) Экспорт
	...
	Если Не Выборка.Следующий() Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Недостаточно прав для открытия варианта ""%1"".'"), Строка(ВариантСсылка));
	КонецЕсли;
	...
	Если Настройки.Оперативное Тогда
		...
		Если Настройки.ОбщиеДанные Тогда
			...
			Если Настройки.Конфигурация Тогда
				...
				Если ПромежуточныйРезультат.ЕстьИзменения Тогда
...
//
// Параметры:
//   Параметры - Структура - Настройки отчета.
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	Результат = Новый Структура("ТекстОшибки, Расшифровка, НастройкиКД, ПользовательскиеНастройкиКД");
	
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(Результат, Параметры, Истина);
	...
	ПодключитьОтчетОбъект  = Не Результат.Свойство("ОтчетОбъект");
	ЗачитатьОтчетСсылка    = ПодключитьОтчетОбъект И Не Результат.Свойство("ОтчетСсылка");
	ЗачитатьОбщиеНастройки = Не Результат.Свойство("НастройкиКД");
	ЗачитатьКлючВарианта   = Не Результат.Свойство("КлючВарианта");
	
	Если ЗачитатьОтчетСсылка Или ЗачитатьОбщиеНастройки Или ЗачитатьКлючВарианта Тогда
		ВариантСсылка = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "ВариантСсылка");
		Если ВариантСсылка = Неопределено Тогда
			Если ЗачитатьОтчетСсылка Тогда
				Результат.ТекстОшибки = СтрШаблон(
					НСтр("ru = 'В методе ""%1"" не указан ни один из параметров ""%2"".'"),
					"СформироватьОтчет",
					"Параметры.ВариантСсылка, Параметры.ОтчетСсылка, Параметры.ОтчетОбъект");
				Возврат Результат;
			КонецЕсли;
			Если ЗачитатьКлючВарианта Тогда
				Результат.Вставить("КлючВарианта", Неопределено);
			КонецЕсли;
		Иначе
			Считываемые = Новый Массив;
			Если ЗачитатьОтчетСсылка Тогда
				Считываемые.Добавить("Отчет");
			КонецЕсли;
			Если ЗачитатьОбщиеНастройки Тогда
				Считываемые.Добавить("Настройки");
			КонецЕсли;
			Если ЗачитатьКлючВарианта Тогда
				Считываемые.Добавить("КлючВарианта");
			КонецЕсли;
			Значения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВариантСсылка, Считываемые);
			Если ЗачитатьОтчетСсылка Тогда
				Результат.Вставить("ОтчетСсылка", Значения.Отчет);
			КонецЕсли;
			Если ЗачитатьОбщиеНастройки Тогда
				НастройкиКД = Значения.Настройки.Получить();
				Если ТипЗнч(НастройкиКД) = Тип("НастройкиКомпоновкиДанных") Тогда
					Результат.Вставить("НастройкиКД", НастройкиКД);
				КонецЕсли;
			КонецЕсли;
			Если ЗачитатьКлючВарианта Тогда
				Результат.Вставить("КлючВарианта", Значения.КлючВарианта);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если ПодключитьОтчетОбъект Тогда
		Подключение = ПодключитьОтчетОбъект(Результат.ОтчетСсылка, Ложь);
		Результат.Вставить("Подключение", Подключение);
		Если Подключение.Успех Тогда
			Результат.Вставить("ОтчетОбъект", Подключение.Объект);
		Иначе
	...
	ОтчетОбъект = Результат.ОтчетОбъект;
	КомпоновщикНастроекКД = ОтчетОбъект.КомпоновщикНастроек;
	
	// При загрузке настроек: 1 проверка на тип, 2 проверка на адрес (не на значение).
	ФиксированныеНастройкиКД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "ФиксированныеНастройкиКД");
	...
	НастройкиКД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "НастройкиКД");
	Если ТипЗнч(НастройкиКД) = Тип("НастройкиКомпоновкиДанных")
		И КомпоновщикНастроекКД.Настройки <> НастройкиКД Тогда
		КомпоновщикНастроекКД.ЗагрузитьНастройки(НастройкиКД);
	КонецЕсли;
	
	ПользовательскиеНастройкиКД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "ПользовательскиеНастройкиКД");
	Если ТипЗнч(ПользовательскиеНастройкиКД) = Тип("ПользовательскиеНастройкиКомпоновкиДанных")
		И КомпоновщикНастроекКД.ПользовательскиеНастройки <> ПользовательскиеНастройкиКД Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДопСвойства, "ВариантМодифицирован") = Истина Тогда
		...
		Результат.Вставить("НастройкиКД", КомпоновщикНастроекКД.Настройки);
	КонецЕсли;
	...
	Если Результат.ВариантМодифицирован
		...
		Результат.Вставить("ПользовательскиеНастройкиКД", КомпоновщикНастроекКД.ПользовательскиеНастройки);
	КонецЕсли;
...
Процедура СформироватьОтчетВФоне(Параметры, АдресХранилища) Экспорт
	Результат = Новый Структура("ТекстОшибки, ТабличныйДокумент, Расшифровка");
	Результат.Вставить("Успех", Ложь);
	Результат.Вставить("ВариантМодифицирован", Ложь);
	Результат.Вставить("ПользовательскиеНастройкиМодифицированы", Ложь);
	...
	Если Формирование.ВариантМодифицирован Тогда
		Результат.Вставить("НастройкиКД", Формирование.НастройкиКД);
	КонецЕсли;
	Если Формирование.ПользовательскиеНастройкиМодифицированы Тогда
...
Функция ДеревоНастроекВариантовОтчетовКонфигурации() Экспорт
	Возврат НастройкиПредопределенных("Внутренний");
КонецФункции
...
//       * СистемнаяИнформация - Структура - Другая служебная информация.
//
Функция НастройкиПредопределенных(ФильтрПоТипуОтчетов) Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		ПостроенНаСКД = (ОтчетМетаданные.ОсновнаяСхемаКомпоновкиДанных <> Неопределено);
		Если ПостроенНаСКД Тогда
			ПодключенКОсновнойФорме = ОтчетПодключенКОсновнойФорме(ОтчетМетаданные, КэшФлажкаОсновнойФормы);
			ПодключенКФормеНастроек = ОтчетПодключенКФормеНастроек(ОтчетМетаданные, КэшФлажкаФормыНастроек);
			Если ПодключенКОсновнойФорме <> ПодключенКФормеНастроек Тогда
				ТекстОшибки = ВариантыОтчетовКлиентСервер.ТекстОшибки(?(ПодключенКОсновнойФорме, 1, 2), ОтчетМетаданные.ПолноеИмя());
				Если ОбщегоНазначенияКлиентСервер.РежимОтладки() Тогда
					ВызватьИсключение ТекстОшибки;
				Иначе
					ОшибкаПоВарианту(ОтчетСсылка, ТекстОшибки);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		СтрокаОтчет.СистемнаяИнформация.Вставить("ПостроенНаСКД", ПостроенНаСКД);
		...
		Если СтрокаОтчет.СистемнаяИнформация.ПостроенНаСКД Тогда
			...
			Исключение
				...
				ТекстОшибки = СтрШаблон(ТекстОшибки, ОтчетМетаданные.Имя);
...
Функция ПоУмолчаниюВсеПодключеныКОсновнойФорме() Экспорт
...
// Определяет способ подключения общей формы отчета.
Функция ПоУмолчаниюВсеПодключеныКФормеНастроек() Экспорт
...
// [*] Актуализирует данные справочника ВариантыОтчетов в части отчетов расширений.
Процедура ОперативноеОбновлениеРазделенныхДанныхРасширений() Экспорт
	
	ОперативноеОбновлениеРазделенныхДанных("РазделенныеДанныеРасширений");
...
Функция ОперативноеОбновлениеОбщихДанных(Режим, РазделенныеОбработчики) Экспорт
	...
	Кэш.Вставить("ДеревоВариантов", НастройкиПредопределенных(?(Кэш.ОбновлятьКонфигурацию, "Внутренний", "Расширение")));
...
Функция ОперативноеОбновлениеРазделенныхДанных(Режим) Экспорт
...
Функция ОтложенноеОбновлениеДанных(Режим, Полное) Экспорт
...
Процедура ОбновитьИндексПоиска(Кэш)
	...
	ПредставлениеПроцедуры = СтрШаблон(НСтр("ru = 'Обновление индекса поиска (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Если Кэш.ОбщиеДанные Тогда
		...
		Если Кэш.Режим = "ОбщиеДанныеКонфигурации" Тогда
			ДеревоВариантов = НастройкиПредопределенных("Внутренний");
			Запрос.Текст =
			"ВЫБРАТЬ
		...
		ИначеЕсли Кэш.Режим = "ОбщиеДанныеРасширений" Тогда
			ДеревоВариантов = НастройкиПредопределенных("Расширение");
			Запрос.Текст =
			"ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Кэш.ОбщиеДанные Тогда
			...
			Если ОтчетОбъект = Неопределено Тогда
				ОтчетОбъект = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтрокаОтчет.СистемнаяИнформация, "ОтчетОбъект");
			КонецЕсли;
...
Процедура АктуализироватьКлючиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтрШаблон(НСтр("ru = 'Обновление ключей вариантов отчетов (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ОбновитьНастройкиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтрШаблон(НСтр("ru = 'Обновление настроек предопределенных (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура АктуализироватьСоставОбласти(Кэш)
	ПредставлениеПроцедуры = СтрШаблон(НСтр("ru = 'Обновление вариантов отчетов (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш)
	ПредставлениеПроцедуры = СтрШаблон(НСтр("ru = 'Удаление вариантов удаленных отчетов (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗарегистрироватьЗамерыВариантаКОбновлению(Кэш, Знач СтарыйКлюч, Знач АктуальныйКлюч, Знач АктуальноеНаименование)
	...
	ОбновлениеЗамера.АктуальноеНаименование = СтрШаблон(НСтр("ru = 'Отчет ""%1"" (открытие)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Формирование";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Формирование";
	ОбновлениеЗамера.АктуальноеНаименование = СтрШаблон(НСтр("ru = 'Отчет ""%1"" (формирование)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Настройки";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Настройки";
	ОбновлениеЗамера.АктуальноеНаименование = СтрШаблон(НСтр("ru = 'Отчет ""%1"" (настройки)'"), АктуальноеНаименование);
КонецПроцедуры
...
//
Процедура ПриОбновленииКлючевыхОпераций(ТаблицаОбновленияКлючевыхОпераций) Экспорт
	ТекстЗапроса =
	"ВЫБРАТЬ
...
Процедура ЗаписатьПараметрыВариантовОтчетов(Кэш)
	...
	Если Кэш.Режим = "ОбщиеДанныеКонфигурации" Тогда
		...
		КонецЕсли;
	ИначеЕсли Кэш.Режим = "ОбщиеДанныеРасширений" Тогда
		ИмяПодсистемы = ВариантыОтчетовКлиентСервер.ПолноеИмяПодсистемы();
		ХранилищеЗначения = Новый ХранилищеЗначения(НовоеЗначение);
		СтандартныеПодсистемыСервер.УстановитьПараметрРаботыРасширения(ИмяПодсистемы, ХранилищеЗначения);
	КонецЕсли;
...
Функция ПроиндексироватьСодержимоеСхемы(ВариантОбъект) Экспорт
	...
	Если ОтчетОбъект = Неопределено Тогда
		...
		Если Подключение.Успех Тогда // Отчет не найден.
			ОтчетОбъект = Подключение.Объект;
...
Функция НайтиСсылки(ПараметрыПоиска) Экспорт
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		|			РазмещениеКонфигурации.Подсистема.Синоним КАК НаименованиеПодсистемы
		|		ИЗ
		|			Варианты КАК ВариантыОтчетов
		...
		|			РазмещениеРасширений.Подсистема.Синоним
		|		ИЗ
		|			Варианты КАК ВариантыОтчетов
		...
		Если Не ЕстьСтрокаПоиска И Не ПолучатьИтоговуюТаблицу Тогда
			...
			ТекстЗапроса = СтрЗаменить(
				ТекстЗапроса,
				"ВариантыРазмещение.Подсистема.Синоним",
				"НЕОПРЕДЕЛЕНО");
			ТекстЗапроса = СтрЗаменить(
				ТекстЗапроса,
				"РазмещениеКонфигурации.Подсистема.Синоним",
				"НЕОПРЕДЕЛЕНО");
			ТекстЗапроса = СтрЗаменить(
				ТекстЗапроса,
				"РазмещениеРасширений.Подсистема.Синоним",
