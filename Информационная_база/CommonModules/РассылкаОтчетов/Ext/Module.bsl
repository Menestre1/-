...
Процедура ВыполнитьРассылкиВФоновомЗадании(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	ИначеЕсли Всего <= 5 Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаРассылок Цикл
			...
			КонецЕсли;
			ШаблонСообщения = СтрШаблон(ШаблонСообщения, СтрокаТаблицы.Представление);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.Справочники.РассылкиОтчетов) 
		Или МодульТекущиеДелаСервер.ДелоОтключено("ПроблемыСРассылкамиОтчетов") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		ИдентификаторМоихЗадач = "ПроблемыСРассылкамиОтчетов" + СтрЗаменить(Раздел.ПолноеИмя(), ".", "");
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	// Определение структуры настроек. Общие параметры.
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "Имя");
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "ПолноеИмя");
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "КлючВарианта");
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "СКД");
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "Метаданные");
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "Объект");
	// Определение структуры настроек. Параметры СКД отчетов.
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "СхемаКД");
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "КомпоновщикНастроекКД");
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "АдресСхемыКД");
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "НастройкиКД");
	// Определение структуры настроек. Параметры произвольных отчетов.
	ВариантыОтчетовКлиентСервер.ДополнитьСтруктуруКлючом(ПараметрыОтчета, "ДоступныеРеквизиты");
	...
	Если ПараметрыОтчета.ЭтоВариант Тогда
		...
		Если РассылкаОтчетовПовтИсп.ИсключаемыеОтчеты().Найти(ЗначенияРеквизитов.Отчет) <> Неопределено Тогда
			ПараметрыОтчета.Ошибки = СтрШаблон(
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено) Экспорт
	...
	ПараметрыФормирования.Вставить("ВариантСсылка", ПараметрыОтчета.Отчет);
	ПараметрыФормирования.Вставить("ОтчетОбъект", ОтчетОбъект);
	
	Если ПараметрыОтчета.СКД Тогда
		ПараметрыФормирования.Вставить("НастройкиКД", Неопределено);
...
Функция КоличествоПроблемСРассылкамиОтчетов()
	...
	Запрос.Текст = 
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РассылкиОтчетов КАК РассылкиОтчетов
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		Запрос.Текст = Запрос.Текст + "	И РассылкиОтчетов.Автор = &Автор";
	КонецЕсли;
