...
Функция ПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля) Экспорт
	...
	Если ПрофилиБезопасности.Количество() <> 1 Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'В кластере серверов %1 не зарегистрирован профиль безопасности %2!'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Процедура СоздатьПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач СвойстваПрофиляБезопасности) Экспорт
	...
	Если ПрофилиБезопасности.Количество() = 1 Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'В кластере серверов %1 уже зарегистрирован профиль безопасности %2!'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Функция ПолучитьИнформационнуюБазу(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИнформационнойБазы) Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'В кластере серверов %1 не зарегистрирована информационная база %2!'"), ИдентификаторКластера, ПараметрыАдминистрированияИнформационнойБазы.ИмяВКластере);
	КонецЕсли;
...
Функция ПолучитьКластер(Знач ПараметрыАдминистрированияКластера) Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Не обнаружен кластер серверов с портом %1'"), ПараметрыАдминистрированияКластера.ПортКластера);
	КонецЕсли;
