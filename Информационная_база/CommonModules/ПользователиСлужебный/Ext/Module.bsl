...
Процедура ПриПолученииОбъектовИсключенийПланаОбмена(Объекты, Знач РаспределеннаяИнформационнаяБаза) Экспорт
	...
	Объекты.Добавить(Метаданные.РегистрыСведений.СведенияОПользователях);
...
Процедура ПриЗаполненииИспользованияВидаДоступа(ИмяВидаДоступа, Использование) Экспорт
	...
	Если ИмяВидаДоступа = "ВнешниеПользователи" Тогда
		Использование = Константы.ИспользоватьВнешнихПользователей.Получить();
	КонецЕсли;
...
Функция ПараметрыПароля(НаименьшаяДлина = 7, Сложный = Ложь) Экспорт
	...
	ПараметрыПароля.Вставить("МаксимальнаяДлина",               32);
...
Функция ПарольОтвечаетТребованиямСложности(Пароль) Экспорт
	...
	НачатьТранзакцию();
	Попытка
		Если ТекущаяМинимальнаяДлина = Истина Тогда
			УстановитьМинимальнуюДлинуПаролейПользователей(0);
		КонецЕсли;
		Если ТекущаяПроверкаСложности = Истина Тогда
		...
		ВременныйПользовательИБ.Имя = СтрШаблон(
...
Функция СохраняемоеЗначениеСтрокиПароля(Знач Пароль,
	...
	Попытка
		...
		ВременныйПользовательИБ.Имя = СтрШаблон(НСтр("ru = 'Временный пользователь (%1)'"),
			Новый УникальныйИдентификатор);
...
Функция ПеремещениеПользователяВНовуюГруппу(МассивПользователей, ГруппаИсточник,
	...
	Если МассивПеремещенныхПользователей.Количество() = 0 И МассивНеПеремещенныхПользователей.Количество() = 0 Тогда
		...
		СообщениеПользователю.Сообщение = СтрШаблон(ТекстСообщения,
...
Функция ФормированиеСообщенияПользователю(МассивПользователей, ГруппаПриемник,
	...
	Если СообщениеПользователю <> Неопределено Тогда
		СообщениеПользователю = СтрШаблон(СообщениеПользователю,
...
Процедура ПроверитьПраваПользователя(ПользовательИБ, РежимПроверки)
	...
	Если УдаляемыеРоли.Количество() = 1 Тогда
		ТекстСообщенияУдаления = СтрШаблон(
	...
	Если ЗначениеЗаполнено(ТекстСообщенияУдаления) Тогда
		ВызватьИсключение ТекстСообщенияУдаления + Символы.ПС + ТекстСообщенияДобавления;
	Иначе
...
Процедура ЗаписатьПользователяИБ(ПользовательОбъект, ПараметрыОбработки)
	...
	Если Не ПараметрыОбработки.УровеньДоступа.ПолныеПрава
	   ...
	   И НЕ (ПараметрыОбработки.УровеньДоступа.УправлениеСписком
	         И Не Пользователи.ВходВПрограммуРазрешен(НоваяАутентификация)) Тогда
