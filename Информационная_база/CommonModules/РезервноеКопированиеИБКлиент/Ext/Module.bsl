...
Процедура ОповеститьПользователяОРезервномКопировании(ВариантОповещения)
	...
	ИначеЕсли ВариантОповещения = "ЕщеНеНастроено" Тогда
		...
		ИмяФормыНастроек = СтрШаблон(ИмяФормыНастроек, ИмяФормыНастроекРезервногоКопирования());
...
Функция КлиентскиеПараметрыРезервногоКопирования() Экспорт
	#Если НЕ ВебКлиент Тогда
		...
		// Определение каталога временных файлов. Использование метода ТекущаяДата оправдано,
		// т.к. резервное копирование не работает в клиент-серверном режиме.
		КаталогВременныхФайловОбновления = КаталогВременныхФайлов() + "1Cv8Backup." + Формат(ТекущаяДата(), "ДФ=ггММддЧЧммсс") + "\";
