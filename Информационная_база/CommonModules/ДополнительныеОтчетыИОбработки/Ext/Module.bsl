...
Процедура ПриПодключенииДопОтчета(Ссылка, ПараметрыОтчета, Результат, ПолучатьМетаданные) Экспорт
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ПараметрыОтчета.ТекстОшибки = СтрШаблон(
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет
		...
		Исключение
			ПараметрыОтчета.ТекстОшибки = 
				СтрШаблон(НСтр("ru = 'При подключении дополнительного отчета ""%1"" возникла ошибка:'"), Строка(Ссылка))
...
Процедура ПриПодключенииОтчета(Контекст) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		Текст    = СтрШаблон(НСтр("ru = 'Отчет ""%1"" не подключен'"), Строка(Ссылка));
		Подробно = СтрШаблон(НСтр("ru = 'Подсистема ""%2"" отключена в настройках программы.
			|Для включения подсистемы обратитесь к администратору программы.'"),
			ДополнительныеОтчетыИОбработкиКлиентСервер.НаименованиеПодсистемы(Истина));
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(Контекст, Текст, Подробно);
		Возврат;
	...
	Иначе
		...
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(Контекст,
			СтрШаблон(НСтр("ru = 'Объект ""%1"" не является дополнительным отчетом'"), Строка(Ссылка)));
...
// Устарела: Следует использовать ПодключенныеОбъектыМетаданных().
Функция НазначаемыеОбъектыМетаданныхПоВидуВнешнегоОбъекта(Вид) Экспорт
	Назначение = ПодключенныеОбъектыМетаданных(Вид);
	Если Назначение = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	Назначение.Колонки.Метаданные.Имя = "ОбъектМетаданных";
	Назначение.Колонки.ПолноеИмя.Имя = "ПолноеИмяОбъектаМетаданных";
	Назначение.Колонки.Ссылка.Имя = "ОбъектНазначения";
	Назначение.Колонки.Вид.Имя = "ВидОбъектаМетаданных";
	Возврат Назначение;
КонецФункции
...
Функция ПредставлениеРаздела(Раздел) Экспорт
	...
	Возврат СтрШаблон(НСтр("ru = 'Раздел ""%1""'"), ПредставлениеРаздела);
КонецФункции
...
Функция ВыполнитьКомандуВнешнегоОбъекта(ВнешнийОбъект, ИдентификаторКоманды, ПараметрыКоманды, АдресРезультата)
	...
	Если ОписаниеКоманды = Неопределено Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Команда %1 не обнаружена!'"), ИдентификаторКоманды);
	...
	Для Каждого КомандаОписание Из РегистрационныеДанные.Команды Цикл
		...
		Если НЕ ЗначениеЗаполнено(КомандаОписание.ВариантЗапуска) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Для команды ""%1"" не определен способ запуска.'"), КомандаОписание.Представление));
		КонецЕсли;
