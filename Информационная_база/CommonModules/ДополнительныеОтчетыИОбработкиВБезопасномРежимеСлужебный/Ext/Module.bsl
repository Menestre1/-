...
Функция ВыполнитьСценарийБезопасногоРежима(Знач КлючСессии, Знач Сценарий, Знач ИсполняемыйОбъект, ПараметрыВыполнения, СохраняемыеПараметры = Неопределено, ОбъектыНазначения = Неопределено) Экспорт
	...
	Для Каждого ШагСценария Из Сценарий Цикл
		...
		Иначе
			...
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный вид действия для этапа сценария: %1'"), ШагСценария.ВидДействия);
		...
		Для Каждого ПараметрМетода Из ПараметрыМетода Цикл
			...
			Иначе
				...
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный параметр для этапа сценария: %1'"), ПараметрМетода.Вид);
	...
	Если ЕстьПараметры Тогда
		...
		Для Каждого Параметр Из ОписанияПараметров Цикл
			...
			Если ЗначениеЗаполнено(ЗначениеПараметра) Тогда
				...
				ОписаниеПараметра = СтрШаблон(Параметр.Описание, ЗначениеПараметра);
				
			Иначе
				
				ОписаниеПараметра = СтрШаблон("<B>%1</B>", Параметр.ОписаниеЛюбогоЗначения);
				
			КонецЕсли;
			
			Результат = Результат + СтрШаблон("<LI><FONT size=2>%1</FONT>", ОписаниеПараметра);
...
Функция ТекстИсключенияРазрешениеНеПредоставлено(Знач КлючСессии, Знач ТипТребуемогоРазрешения)
	...
	Возврат СтрШаблон(НСтр("ru = 'Дополнительному отчету или обработке %1 не предоставлено разрешение {%2}%3!'"),
