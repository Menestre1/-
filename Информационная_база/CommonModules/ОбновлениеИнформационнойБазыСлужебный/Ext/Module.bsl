...
Функция ВыполнитьОбновлениеИнформационнойБазы(ПараметрыОбновления) Экспорт
	...
	Попытка
		...
		Если РежимВыполненияОтложенногоОбновления <> Неопределено
			И РежимВыполненияОтложенногоОбновления = "Монопольно" Тогда
...
Функция РежимОбновленияДанных() Экспорт
	...
	Если НЕ СтандартнаяОбработка Тогда
		...
		Сообщение = СтрШаблон(НСтр("ru = 'Недопустимое значение параметра %1 в %2. 
...
Процедура СобратьСтатистикуКонфигурации() Экспорт
	...
	МодульЦентрМониторинга.ЗаписатьСтатистикуОбъектаКонфигурации("ВремяВыполненияОбработчиков",
		СведенияОбОбновлении.ВремяОкончанияОбновления - СведенияОбОбновлении.ВремяНачалаОбновления);
		
	МодульЦентрМониторинга.ЗаписатьСтатистикуОбъектаКонфигурации("ВремяВыполненияОтложенныхОбработчиков",
		СведенияОбОбновлении.ВремяОкончаниеОтложенногоОбновления - СведенияОбОбновлении.ВремяНачалаОтложенногоОбновления);
...
Функция ИтерацииОбновления()
	...
	Если ИтерацияОбновлениеОсновнойПодсистемы = Неопределено Тогда
		...
		Иначе
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Сведения об основной конфигурации ""%1"" не заданы.
...
Функция ВыполнитьИтерациюОбновления(Знач ИтерацияОбновления, Знач Параметры) Экспорт
	...
	Если ТекущаяВерсияИБ <> "0.0.0.0"
		...
		Если ВыполняемыеОбработчики = Неопределено Тогда
			...
			КонецЕсли;
			Сообщение = СтрШаблон(ШаблонСообщения, ИдентификаторБиблиотеки, ТекущаяВерсияИБ, ВерсияМетаданных);
			ЗаписатьИнформацию(Сообщение);
	...
	Для Каждого Версия Из ВыполняемыеОбработчики.Строки Цикл
		...
		Иначе
			...
			ИначеЕсли ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтрШаблон(НСтр("ru = 'Выполняется обновление информационной базы с версии %1 на версию %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтрШаблон(НСтр("ru = 'Выполняется обновление данных библиотеки %3 с версии %1 на версию %2.'"), 
		...
		Иначе
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтрШаблон(НСтр("ru = 'Выполнено обновление информационной базы с версии %1 на версию %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтрШаблон(НСтр("ru = 'Выполнено обновление данных библиотеки %3 с версии %1 на версию %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ, ИдентификаторБиблиотеки);
			КонецЕсли;
		КонецЕсли;
		ЗаписатьИнформацию(Сообщение);
...
Процедура СформироватьСписокОтложенныхОбработчиковОбновления(ИтерацииОбновления)
	...
	СведенияОбОбновлении.Вставить("ФорсироватьОтложенноеОбновление", Ложь);
		...
		Для Каждого СтрокаВерсия Из ОбработчикиПоВерсиям.Строки Цикл
			...
			Для Каждого СтрокаОбработчики Из СтрокаВерсия.Строки Цикл
				...
				Если СтрокаОбработчики.МонопольныйРежим = Истина Тогда
					...
					ТекстОшибки = СтрШаблон(НСтр("ru = 'У отложенного обработчика ""%1""
				...
				СтрокаДереваОбработчики.Идентификатор = СтрокаОбработчики.Идентификатор;
...
Процедура ВыполнитьОтложенноеОбновление() Экспорт
	...
	Пока ОбработчикиВыполнялись Цикл
		...
		Если СведенияОбОбновлении.ФорсироватьОтложенноеОбновление Тогда
			СведенияОбОбновлении = СведенияОбОбновленииИнформационнойБазы();
		Иначе
			Прервать;
		КонецЕсли;
...
Функция КоличествоОбработчиковНаТекущуюВерсию(ИтерацииОбновления)
	...
	Сообщение = СтрШаблон(Сообщение, КоличествоОбработчиков);
	ЗаписатьИнформацию(Сообщение);
...
Функция ВыполнитьОтложенныйОбработчикОбновления(СведенияОбОбновлении, ВыполнитьНеудачные = Ложь)
	...
	Для Каждого ПриостановленныйОбработчик Из СведенияОбОбновлении.ПриостановленныеПроцедурыОбновления Цикл
		НайденныйОбработчик = СведенияОбОбновлении.ДеревоОбработчиков.Строки.Найти(ПриостановленныйОбработчик, "ИмяОбработчика", Истина);
	...
	Для Каждого ЗапущенныйОбработчик Из СведенияОбОбновлении.ЗапущенныеПроцедурыОбновления Цикл
	...
	Если СведенияОбОбновлении.ПриостановленныеПроцедурыОбновления.Количество() > 0
		Или СведенияОбОбновлении.ЗапущенныеПроцедурыОбновления.Количество() > 0 Тогда
		СведенияОбОбновлении.ПриостановленныеПроцедурыОбновления.Очистить();
		СведенияОбОбновлении.ЗапущенныеПроцедурыОбновления.Очистить();
		ЗаписатьСведенияОбОбновленииИнформационнойБазы(СведенияОбОбновлении);
	КонецЕсли;
	...
	Для Каждого ДеревоОбработчиковБиблиотека Из СведенияОбОбновлении.ДеревоОбработчиков.Строки Цикл
		...
		Для Каждого ДеревоОбработчиковВерсия Из ДеревоОбработчиковБиблиотека.Строки Цикл
			...
			Для Каждого ОбработчикОбновления Из ДеревоОбработчиковВерсия.Строки Цикл
				...
				Попытка
					СообщениеОЗапущенномОбработчике = СтрШаблон(НСтр("ru = 'Выполняется процедура обновления ""%1"".'"), ИмяОбработчика);
				...
				Иначе
					...
					Попытка
						...
						СведенияОбОбновлении.ПриостановленныеПроцедурыОбновления = НовыеСведенияОбОбновлении.ПриостановленныеПроцедурыОбновления;
						СведенияОбОбновлении.ЗапущенныеПроцедурыОбновления       = НовыеСведенияОбОбновлении.ЗапущенныеПроцедурыОбновления;
						СведенияОбОбновлении.ФорсироватьОтложенноеОбновление     = НовыеСведенияОбОбновлении.ФорсироватьОтложенноеОбновление;
...
Функция ПереданныеПараметрыОбработчикаОбновления(Параметры)
	...
	Для Каждого Параметр Из Параметры Цикл
		Если Параметр.Ключ <> "ОбработкаЗавершена"
			И Параметр.Ключ <> "ПрогрессВыполнения" Тогда
