	...
	Если ПодключитьРасширениеРаботыСФайлами() Тогда
		...
		Если ЗначениеЗаполнено(ВыбранныйФайл.Имя) Тогда
			...
			ИначеЕсли РасширениеФайла = "xlsx" Тогда
				АдресВоВременномХранилище = ШаблонТабличногоДокумента();
...
//
&НаСервере
Функция ШаблонТабличногоДокумента()
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	ТекстВсе = СтрШаблон(НСтр("ru = 'Все (%1)'"), Статистика.Всего);
	...
	СписокВыбора.Добавить("Несопоставленные", СтрШаблон(НСтр("ru = 'Несопоставленные (%1 из %2)'"),
		Статистика.НеСопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Сопоставленные", СтрШаблон(НСтр("ru = 'Сопоставленные (%1 из %2)'"),
		Статистика.Сопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Неоднозначные", СтрШаблон(НСтр("ru = 'Неоднозначные (%1 из %2)'"),
	...
	Если Статистика.Неоднозначных > 0 Тогда 
		...
		Элементы.ОписаниеНеоднозначность.Заголовок = СтрШаблон(НСтр("ru = '(неоднозначностей: %1)'"),
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Тогда
			...
			Элементы.НадписьРезультатСопоставления.Заголовок = СтрШаблон(ТекстНайдено,
			...
			Если Статистика.Неоднозначных > 0 И Статистика.НеНайдено > 0 Тогда 
				...
				ТекстНеНайдено = СтрШаблон(ТекстНеНайдено, Статистика.НеНайдено, Статистика.Неоднозначных);
