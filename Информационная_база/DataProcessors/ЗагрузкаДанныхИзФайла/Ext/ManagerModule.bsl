...
Функция СтрокаЗапроса(ТекстЗапроса, ТипОбъекта, ИмяОбъекта, МассивКолонок)
	...
	Если МассивКолонок.Количество() > 0 Тогда
		...
		Иначе
			ТекстОбъединитьВсе = "";
		КонецЕсли;
		ТекстЗапроса = ТекстЗапроса + ТекстОбъединитьВсе + СтрШаблон(ТекстШаблон, ИмяОбъекта, ТипОбъекта);
	КонецЕсли;
...
Процедура ИнформациюПоКолонкамИзРеквизитовСправочника(ПараметрыЗагрузки, ИнформацияПоКолонкам) Экспорт
	...
	Для каждого Реквизит Из МетаданныеСправочника.Реквизиты Цикл
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Число")) Тогда 
			ОписаниеТипаКолонки =  СтрШаблон(НСтр("ru = 'Число, Длина: %1, Точность: %2'"),
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Строка")) Тогда
			Если Реквизит.Тип.КвалификаторыСтроки.Длина > 0 Тогда
				...
				ОписаниеТипаКолонки =  СтрШаблон(НСтр("ru = 'Строка, макс. количество символов: %1'"), ДлинаСтроки);
			Иначе
...
Процедура СоздатьКолонкуСтандартныеРеквизиты(ИнформацияПоКолонкам, МетаданныеСправочника, ИмяКолонки, Позиция)
	...
	Если ТипДанных = Тип("Строка") Тогда 
		ПредставлениеТипа = СтрШаблон(НСтр("ru = 'Строка (не более %1 символов)'"), ОписаниеТипа.КвалификаторыСтроки.Длина);
...
Процедура ИнициализироватьЗагрузкуВТабличнуюЧасть(ПолноеИмяТабличнойЧасти, ИмяМакетаСШаблоном, ИнформацияПоКолонкам, ШаблонСДанными, ДополнительныеПараметры, Отказ) Экспорт
	...
	// Тестирование
	ИначеЕсли СтруктураНаименованияОбъекта.ТипОбъекта = "Справочник" Тогда
		МенеджерОбъекта = Справочники[СтруктураНаименованияОбъекта.НазваниеОбъекта];
		...
		МакетМетаданные = Метаданные.Справочники[СтруктураНаименованияОбъекта.НазваниеОбъекта].Макеты.Найти(ИмяМакетаСШаблоном);
		Если МакетМетаданные = Неопределено Тогда 
			МакетМетаданные= Метаданные.Справочники[СтруктураНаименованияОбъекта.НазваниеОбъекта].Макеты.Найти("ЗагрузкаИзФайла" + СтруктураНаименованияОбъекта.ИмяТабличнойЧасти);
			Если МакетМетаданные = Неопределено Тогда 
				МакетМетаданные = Метаданные.Справочники[СтруктураНаименованияОбъекта.НазваниеОбъекта].Макеты.Найти("ЗагрузкаИзФайла");
			КонецЕсли;
		КонецЕсли;
...
Функция МенеджерОбъекта(ИмяОбъектаСопоставления)
		...
		Иначе
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Объект ""%1"" не найден'"), ИмяОбъектаСопоставления);
		КонецЕсли;
		...
		Попытка
			...
			Иначе
				...
				Если ЭлементСправочника = Неопределено Тогда
					ВызватьИсключение СтрШаблон(НСтр("ru = 'Номенклатура с артикулом %1 не существует.'"),
