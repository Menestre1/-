...
Процедура ПоказатьСвязиНеУдаленныхНаКлиенте()
	...
	Иначе
		...
		НеУдаленныеПодсказка = СтрШаблон(Шаблон, СтрокаДерева.Представление, Формат(СтрокаДерева.КоличествоСвязей, "ЧН=0; ЧГ="));
	КонецЕсли;
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдаление(ТаблицаЭлемент)
	...
	Если КоличествоМожноУдалить = 1 Тогда
		...
		КонецЕсли;
		ТекстВопроса = СтрШаблон(ТекстВопроса, МассивСтрокТаблицы[0].Представление);
	Иначе
		Если ЕстьПомеченныеНаУдаление Тогда
		...
		КонецЕсли;
		ТекстВопроса = СтрШаблон(ТекстВопроса, Формат(КоличествоМожноУдалить, "ЧН=0; ЧГ="));
	КонецЕсли;
	...
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		...
		Иначе
			...
			Если УдаленныеКоличество = 0 Тогда
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтрШаблон(
