...
Функция СписокНастроекФорм(СписокФорм, ИмяПользователя)
	...
	Для Каждого Элемент Из СписокФорм Цикл
		...
		Для Каждого НастройкаФормы Из МассивНастроекФорм Цикл
			...
			ПоискНастройки = КлючиНастроек.Найти(Элемент.Значение + НастройкаФормы);
			...
			Для Каждого Настройка Из Элемент Цикл
				...
				Иначе
					...
					Для Каждого ЭлементФорм Из МассивНастроекФорм Цикл
						МенеджерНастроек.Удалить(Настройка.Значение + ЭлементФорм, Настройка.Представление, ПользовательИБ);
					КонецЦикла;
...
Процедура ДобавитьТипФормыВПредставление(Объект, Форма, ИмяФормы)
	...
	Если Форма = Объект.ОсновнаяФормаСписка Тогда
		ИмяФормы = СтрШаблон(НСтр("ru='%1 (список)'"), ИмяФормы);
	ИначеЕсли Форма = Объект.ОсновнаяФормаДляВыбора Тогда
		ИмяФормы = СтрШаблон(НСтр("ru='%1 (выбор)'"), ИмяФормы);
	ИначеЕсли Форма = Объект.ОсновнаяФормаГруппы Тогда
		ИмяФормы = СтрШаблон(НСтр("ru='%1 (группа)'"), ИмяФормы);
	КонецЕсли;
