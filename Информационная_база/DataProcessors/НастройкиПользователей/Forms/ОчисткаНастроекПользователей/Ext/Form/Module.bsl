...
Функция ОчисткаНастроек()
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям" Тогда
		...
		Если КоличествоПользователей = 1 Тогда
			...
			ПояснениеУКогоОчищеныНастройки = СтрШаблон(
		...
		Иначе
			...
			ПояснениеУКогоОчищеныНастройки = СтрШаблон(ПояснениеУКогоОчищеныНастройки, КоличествоПользователей);
		КонецЕсли;
	...
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки" Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			...
			ТекстПояснения = СтрШаблон(ТекстПояснения, ПредставлениеНастройки, ПояснениеУКогоОчищеныНастройки);
		...
		Иначе
			...
			ТекстПояснения = СтрШаблон(ТекстПояснения, ПрописьПредмета, ПояснениеУКогоОчищеныНастройки);
		КонецЕсли;
	...
	ИначеЕсли ПереключательОчищаемыеНастройки = "ОчиститьВсе" Тогда
		...
		ТекстПояснения = СтрШаблон(ТекстПояснения, ПояснениеУКогоОчищеныНастройки);
		ПоказатьОповещениеПользователя(НСтр("ru = 'Очистка настроек'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
	КонецЕсли;
