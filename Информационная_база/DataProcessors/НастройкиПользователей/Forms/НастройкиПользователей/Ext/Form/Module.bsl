...
Процедура ПользовательСсылкаПриИзменении(Элемент)
	...
	Если РезультатПолученияНастроек() = "НетПользователяИБ" Тогда
		...
		ТекстСообщения = СтрШаблон(ТекстСообщения, ПользовательСсылка);
...
Процедура ОчиститьВсеНастройки(Команда)
	...
	ТекстВопроса = СтрШаблон(ТекстВопроса, Строка(ПользовательСсылка));
...
Процедура РассчитатьКоличествоНастроек()
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.НастройкиОтчетовСтраница.Заголовок = СтрШаблон(НСтр("ru='Настройки отчетов (%1)'"), КоличествоНастроек);
	Иначе
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ВнешнийВидСтраница.Заголовок = СтрШаблон(НСтр("ru='Внешний вид (%1)'"), КоличествоНастроек);
	Иначе
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ПрочиеНастройкиСтраница.Заголовок = СтрШаблон(НСтр("ru='Прочие настройки (%1)'"), КоличествоНастроек);
	Иначе
		...
		Для Каждого Настройка Из Результат.МассивНастроек Цикл
			...
			Для Каждого Элемент Из Настройка Цикл
				...
				Если Элемент.Пометка Тогда
					...
					КлючОтчета = СтрШаблон(Элемент.Значение, "/");
	...
	Если ВРег(ИмяСобытия) = ВРег("ВыборПользователя") Тогда
		...
		Если Параметр.КопироватьВсе Тогда
			...
			Если МассивНастроек.Количество() = 7 Тогда
				ТекстПояснения = СтрШаблон(НСтр("ru = 'Скопированы все настройки %1'"), ПояснениеКомуСкопированыНастройки);
			Иначе
				ТекстПояснения = СтрШаблон(НСтр("ru = '%1 скопированы %2'"), ИменаНастроек, ПояснениеКомуСкопированыНастройки);
			КонецЕсли;
...
Процедура ОчиститьВсеНастройкиЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = СтрШаблон(ТекстПояснения, ПользовательСсылка);
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВидаЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = СтрШаблон(ТекстПояснения, Строка(ПользовательСсылка));
	ПоказатьОповещениеПользователя(НСтр("ru = 'Очистка настроек'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	Если КоличествоНастроек = 1
		...
		ТекстПояснения = СтрШаблон(ТекстПояснения, НазваниеНастройки, Строка(ПользовательСсылка));
	ИначеЕсли КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = НСтр("ru = 'Очищено %1 пользователю ""%2""'");
		ТекстПояснения = СтрШаблон(ТекстПояснения, ПрописьПредмета, Строка(ПользовательСсылка));
	КонецЕсли;
	...
	Если КоличествоПользователей <> Неопределено Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			...
			ТекстПояснения = СтрШаблон(ТекстПояснения, НазваниеНастройки, ПояснениеКомуОчищатьНастройки);
		Иначе
			ТекстПояснения = НСтр("ru = 'Очищено %1 %2'");
			ТекстПояснения = СтрШаблон(ТекстПояснения, ПрописьПредмета, ПояснениеКомуОчищатьНастройки);
		КонецЕсли;
