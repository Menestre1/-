...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПриоритетОбновления = ?(СведенияОбОбновлении.ФорсироватьОтложенноеОбновление, "ОбработкаДанных", "РаботаПользователей");
...
Процедура УстановитьПриоритетОбновления()
	...
	Попытка
		...
		СведенияОбОбновлении = ОбновлениеИнформационнойБазыСлужебный.СведенияОбОбновленииИнформационнойБазы();
		СведенияОбОбновлении.ФорсироватьОтложенноеОбновление = ?(ПриоритетОбновления = "ОбработкаДанных", Истина, Ложь);
...
Функция СообщениеОРезультатахОбновления(СведенияОбОбновлении)
	...
	Если ВсегоОбработчиков = УспешноВыполненоОбработчиков Тогда
		...
		Если ВсегоОбработчиков = 0 Тогда
			...
			ТекстСообщения = "";
		Иначе
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Все процедуры обновления выполнены успешно (%1)'"), УспешноВыполненоОбработчиков);
		КонецЕсли;
		Элементы.КартинкаИнформация1.Картинка = БиблиотекаКартинок.Успешно32;
	Иначе
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не все процедуры удалось выполнить (выполнено %1 из %2)'"), 
