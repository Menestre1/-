...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		ТекстЗаголовка = СтрШаблон(ТекстЗаголовка, ДатаПоследнегоКопирования);
	КонецЕсли;
...
Процедура ОтменитьПодготовку()
	...
	Элементы.НадписьНеУдалось.Заголовок = СтрШаблон(НСтр("ru = '%1.
...
Процедура НачатьРезервноеКопирование()
	...
	СтрокаЗапуска = СтрШаблон(СтрокаЗапуска, ИмяГлавногоФайлаСкрипта, РезервноеКопированиеИБКлиент.СтрокаUnicode(ПарольАдминистратораИБ));
