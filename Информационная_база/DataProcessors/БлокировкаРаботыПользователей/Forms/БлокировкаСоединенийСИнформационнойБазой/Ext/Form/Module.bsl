...
Процедура Применить(Команда)
	...
	Если Объект.ВременноЗапретитьРаботуПользователей Тогда
		...
		Иначе
			...
			ТекстВопроса = СтрШаблон(ТекстВопроса, Объект.НачалоДействияБлокировки - 900, Объект.НачалоДействияБлокировки);
...
Процедура ОбновитьСостояниеБлокировки(Форма)
	...
	Форма.Элементы.Состояние.Заголовок = СтрШаблон(НСтр("ru='Пожалуйста, подождите...
