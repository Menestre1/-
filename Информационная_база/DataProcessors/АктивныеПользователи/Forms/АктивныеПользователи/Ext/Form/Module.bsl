...
Функция НайтиСсылкуПоИдентификаторуПользователя(Идентификатор)
	...
	ТекстЗапросаПоПользователям = СтрШаблон(ШаблонТекстаЗапроса, Метаданные.Справочники.Пользователи.ПолноеИмя());
	
	ТекстЗапросаПоВнешнимПользователям = СтрШаблон(ШаблонТекстаЗапроса, Метаданные.Справочники.ВнешниеПользователи.ПолноеИмя());
...
Процедура ПослеЗавершенияСеанса(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если ДополнительныеПараметры.Свойство("НомераСеансов")
			...
			ТекстОповещения = СтрШаблон(ТекстОповещения, НомераСеансов);
		...
		Иначе
			...
			ТекстОповещения = СтрШаблон(НСтр("ru = 'Сеанс %1 завершен.'"), НомерСеанса);
