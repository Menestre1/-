...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВсеПубликацииКромеНеиспользующихся.Количество() > 1 Тогда
		...
		ПредставлениеМассива = СтрШаблон(
			НСтр("ru = '%1 или %2'"),
...
Функция ИзменениеПубликации(ВариантПубликации)
	...
	Если ВыделенныеСтроки.Количество() = 1 Тогда
		...
		ТекстСообщения = СтрШаблон(ТекстСообщения, ИмяОбъекта);
	Иначе
		ТекстСообщения = НСтр("ru = 'Изменена публикация у %1 дополнительных отчетов (обработок)'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, ВыделенныеСтроки.Количество());
	КонецЕсли;
...
Процедура СписокИзменитьПометкуУдаления()
	...
	КонецЕсли;
	ТекстВопроса = СтрШаблон(ТекстВопроса, СтрокаТаблицы.Наименование);
