...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация")
		И ДействияСПользователемСервиса <> Неопределено Тогда
			МодульУправлениеКонтактнойИнформацией = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформацией");
			МодульУправлениеКонтактнойИнформацией.УстановитьДоступностьЭлементовКонтактнойИнформации(ЭтотОбъект, ОпределитьДоступностьЭлементовКонтактнойИнформации());
	КонецЕсли;
	...
	Если ТипЗнч(Объект.ФизическоеЛицо) = Тип("Строка") Тогда
		Элементы.ФизическоеЛицо.Видимость = Ложь;
	КонецЕсли;
	
	Если ТипЗнч(Объект.Подразделение) = Тип("Строка") Тогда
		Элементы.ТекущееПодразделение.Видимость = Ложь;
	КонецЕсли;
...
Процедура ОбщаяНастройкаФормы(ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	Элементы.ТекущееПодразделение.ТолькоПросмотр        = Элементы.Наименование.ТолькоПросмотр;
...
Функция ОпределитьДоступностьЭлементовКонтактнойИнформации()
	...
	Для Каждого СтрокаКонтактнойИнформации Из ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		...
		Если СтрокаКонтактнойИнформации.Вид = ВидКонтактнойИнформацииEmailПользователя() Тогда
			Результат.Вставить(СтрокаКонтактнойИнформации.Вид, Объект.Ссылка.Пустая());
		Иначе
			Результат.Вставить(СтрокаКонтактнойИнформации.Вид, НЕ (ЭлементКонтактнойИнформации.ТолькоПросмотр ИЛИ НЕ ДействияВидаКонтактнойИнформации.Изменение));
		КонецЕсли;
	КонецЦикла;
