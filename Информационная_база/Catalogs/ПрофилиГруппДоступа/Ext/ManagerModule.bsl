	...
	Для каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		...
		Для каждого Роль Из ОписаниеПрофиля.Роли Цикл
			Если ВсеРоли.Получить(Роль) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтрШаблон(
		...
		Для Каждого СтрокаТипа Из ОписаниеПрофиля.Назначение Цикл
			Если ТипЗнч(СтрокаТипа) = Тип("Тип") Тогда
				МассивНазначенияПоставляемый = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СтрокаТипа);
			ИначеЕсли ТипЗнч(СтрокаТипа) = Тип("ОписаниеТипов") Тогда
				МассивНазначенияПоставляемый = СтрокаТипа.Типы();
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтрШаблон(
					НСтр("ru = 'В описании профиля ""%1""
					|указан недопустимый тип назначения ""%2"".
					|Ожидается назначение типа ""Тип"" или ""ОписаниеТипов"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
					ТипЗнч(ОписаниеПрофиля.Назначение));
			КонецЕсли;
			
			Для Каждого Тип Из МассивНазначенияПоставляемый Цикл
				...
				МассивНазначения.Добавить(Значение);
			КонецЦикла;
		КонецЦикла;
		ОписаниеПрофиля.Назначение = МассивНазначения;
	КонецЦикла;
