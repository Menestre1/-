...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтруктураВыбора.СОшибками.Количество > 0 Тогда
		...
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(Результат,
			ТекстСообщения,
			СтруктураВыбора.СОшибками.Текст,
			НСтр("ru = 'Ошибки добавления отчетов в рассылку'"));
	...
	Иначе
		...
		СтандартныеПодсистемыКлиентСервер.ВывестиОповещение(Результат,
			ЗаголовокОповещения,
			ТекстОповещения,
...
Процедура ИспользоватьПолучателяРассылкиВНастройкеОтчета(Команда)
	...
	Если Не Объект.Персонализирована Тогда
		...
		СообщениеТекст = СтрШаблон(СообщениеТекст, ПредставлениеВида);
	...
	Если Не ОписаниеДоступныхТипов.СодержитТип(ОсновнойТипПолучателей) Тогда
		...
		ТекстПредупреждения = СтрШаблон(ТекстПредупреждения, Строка(ОсновнойТипПолучателей));
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Функция ВыбратьФормат(СсылкаНаОтчет, ОбработчикРезультата)
	...
	Иначе
		...
		ЗаголовокДиалога = СтрШаблон(ЗаголовокДиалога, Строка(СсылкаНаОтчет));
	КонецЕсли;
...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "Отчеты" Тогда
		...
		Если КоличествоОтчетов > 0 Тогда
			Элементы.СтраницаОтчеты.Заголовок = СтрШаблон(
				НСтр("ru = 'Отчеты (%1)'"), 
	...
	Если Изменения = ""
		...
		Элементы.СтраницаДоставка.Заголовок = СтрШаблон(НСтр("ru = 'Доставка (%1)'"), СпособыДоставки);
	КонецЕсли;
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(Результат,
		ТекстСообщения,
		РассылкаОтчетовКлиентСервер.СтрокаСообщенийПользователю(ПараметрыЖурнала.МассивОшибок));
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		Если Не ЭтоНовый Тогда
			...
			КонецЕсли;
			ШаблонСтрокиСообщения = СтрШаблон(ШаблонСтрокиСообщения, Строка(ВариантСсылка));
