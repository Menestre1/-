...
Функция РазместитьВариантыВПодсистеме(ПараметрыРазмещения)
	...
	Если ПараметрыРазмещения.Варианты.Всего = Размещено Тогда
		...
		Иначе
			...
			КонецЕсли;
			Текст = СтрШаблон(НСтр("ru = 'Варианты отчетов (%1).'"), Формат(ПараметрыРазмещения.Варианты.Всего, "ЧН=0; ЧГ=0"));
			Ссылка = Неопределено;
		КонецЕсли;
	...
	Иначе
		...
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(РезультатВыполнения,
			СтрШаблон(Шаблон, Формат(Размещено, "ЧН=0; ЧГ=0"), Формат(ПараметрыРазмещения.Варианты.Всего, "ЧН=0; ЧГ=0")),
			ТекстОшибок);
	КонецЕсли;
