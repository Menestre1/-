...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Параметры.ПереданнаяСтрокаПоиска) Тогда
		...
		Если РезультатыПоиска.Количество() <> 0 Тогда
			...
			ПоказаныРезультатыСПо = СтрШаблон(
...
&НаКлиенте
Процедура ОбластиПоискаНажатиеПродолжение(Результат, Параметры) Экспорт
...
Процедура Искать(Направление, Отказ = Неопределено)
	...
	Состояние(СтрШаблон(НСтр("ru = 'Выполняется поиск ""%1""...'"), СтрокаПоиска));
