...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(ВладелецФормы) <> Тип("УправляемаяФорма")
	 Или ВладелецФормы.Окно <> Окно Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = СтрШаблон(НСтр("ru = 'Права доступа (%1)'"), Строка(Параметры.Пользователь));
	КонецЕсли;
...
Процедура ЗаписатьИзмененияНаСервере(Отказ)
	...
	Для Каждого Строка Из ТаблицаЗначений Цикл
		Если ТекущаяГруппаДоступа <> Строка.ГруппаДоступа Тогда
			ТекущаяГруппаДоступа = Строка.ГруппаДоступа;
