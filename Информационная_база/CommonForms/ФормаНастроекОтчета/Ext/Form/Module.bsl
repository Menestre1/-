...
Процедура ПодсказкаЕстьВложенныеОтчетыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыФормы, ВладелецФормы.ФормаПараметры, Истина);
...
Процедура Подключаемый_СтандартныйПериод_Вид_ПриИзменении(Элемент)
	...
	СвойстваНастройкиТип = Лев(ВидПериодаИмя, СтрНайти(ВидПериодаИмя, "_Вид_")-1);
...
Процедура Подключаемый_СтандартныйПериод_Значение_НачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	СвойстваНастройкиТип = Лев(ПредставлениеИмя, СтрНайти(ПредставлениеИмя, "_Представление_")-1);
...
Процедура Подключаемый_СтандартныйПериод_НачалоПериода_ПриИзменении(Элемент)
	...
	СвойстваНастройкиТип = Лев(ПериодНачалоИмя, СтрНайти(ПериодНачалоИмя, "_Начало_")-1);
	ЗначениеИмя = СвойстваНастройкиТип + "_Значение_" + ИдентификаторЭлемента;
...
Процедура Подключаемый_СтандартныйПериод_КонецПериода_ПриИзменении(Элемент)
	...
	СвойстваНастройкиТип = Лев(ПериодОкончаниеИмя, СтрНайти(ПериодОкончаниеИмя, "_Окончание_")-1);
...
Функция ВыполнитьШаг(ВыборПользователя, Сценарий)
	...
	Иначе
		Сценарий.ПричинаОтказа = СтрШаблон(НСтр("ru = 'Ошибка в сценарии: Неизвестный шаг ""%1"".'"), Сценарий.ТекущийШаг);
		Возврат Ложь;
...
Процедура СтруктураВариантаИзменитьУзел(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы, ИмяСтраницы)
	...
	ПараметрыФормы.Вставить("Заголовок", СтрШаблон(ШаблонЗаголовка, ПредставлениеУзла, ВариантНаименование));
...
Процедура ОформлениеИзменитьЭлемент(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы)
	...
	КонецЕсли;
	ПараметрыФормы.Вставить("Заголовок", СтрШаблон(НСтр("ru = 'Элемент условного оформления отчета ""%1""'"), ВариантНаименование));
...
Функция ТаблицыПолейПереместить(Знач ТаблицаИмя, Знач СтрокаТаблицы, Знач НовыйРодитель, Знач ПередЧемВставить, Знач ОставитьКопию)
	...
	Если ПередЧемВставить = Неопределено Тогда
		Если ТаблицаИмя = "СтруктураВарианта"
			...
			Или ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НовыйРодитель, "ЭтоРаздел") = Истина
			Или ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НовыйРодитель, "ЭтоГруппа") = Истина Тогда
			// Внутрь родителя.
		Иначе
...
Функция ТаблицыПолейНайтиУзел(ЭтотОбъект, ТаблицаИмя, СтрокаТаблицы = Неопределено, ИдентификаторУзлаКД = Неопределено)
	Если ЭтотОбъект.РасширенныйРежим = 1 Тогда
		...
		Иначе
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Изменение узлов таблицы ""%1"" не поддерживается.'"), ТаблицаИмя);
		КонецЕсли;
	Иначе
...
Процедура РасширенныеНастройкиЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	Иначе
		ВидимостьУсловийОтборов = РасширенныйРежим = 0 И (Информация.ЕстьБыстрыеНастройки Или Информация.ЕстьОбычныеНастройки);
		...
		Элементы.ИзменитьУсловияОтборов.Видимость              = ВидимостьУсловийОтборов;
