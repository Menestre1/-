...
&НаКлиенте
Процедура ОбработкаОповещения(ИмяГлобальногоСобытия, Изменения, Источник)
	Если Источник = ЭтотОбъект Тогда
		Возврат;
	КонецЕсли;
	ИмяЛокальногоСобытия = Неопределено;
	Если ИмяГлобальногоСобытия = ВариантыОтчетовКлиентСервер.ИмяСобытияИзменениеВарианта() Тогда
		ИмяЛокальногоСобытия = "";
	ИначеЕсли ИмяГлобальногоСобытия = ВариантыОтчетовКлиентСервер.ИмяСобытияИзменениеОбщихНастроек() Тогда
	...
	КонецЕсли;
	Если ИмяЛокальногоСобытия <> Неопределено Тогда
		ОбновитьПанельОтчетовНаКлиенте(ИмяЛокальногоСобытия);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьПанельОтчетовНаКлиенте(Событие = "")
	...
	Результат = ОбновитьПанельОтчетовНаСервере(Событие);
...
Процедура ЗаполнитьПанельОтчетов()
	...
	Иначе
		...
		Если ПараметрыЗаполнения.НеВыведено > 0 Тогда // Вывод информационной надписи.
			...
			ЗаголовокНадписи = СтрШаблон(ЗаголовокНадписи, ПараметрыЗаполнения.ПределВывода);
