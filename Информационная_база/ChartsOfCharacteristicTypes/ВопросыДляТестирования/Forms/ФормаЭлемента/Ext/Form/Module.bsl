...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Выделим верные варианты ответов в динамическом списке справочника "Варианты ответов тестов"
	МассивВерныхВариантовОтветов = Новый Массив();
    Для Каждого Строка Из Объект.ВерныеОтветы Цикл
		МассивВерныхВариантовОтветов.Добавить(Строка.Ответ);
	КонецЦикла;
	ВариантыОтветов.Параметры.УстановитьЗначениеПараметра("МассивВерныхВариантовОтветов", МассивВерныхВариантовОтветов);
	...
	Если Объект.ВерныеОтветы.Количество() > 0 Тогда
		ВерныйОтвет = Объект.ВерныеОтветы[0].Ответ; 
		Если Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.Строка") Тогда 
			ВерныйОтветСтрока = ВерныйОтвет;
		ИначеЕсли Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.Число") Тогда
			ВерныйОтветЧисло = ВерныйОтвет;
		ИначеЕсли Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.Дата") Тогда
			ВерныйОтветДата = ВерныйОтвет;
		ИначеЕсли Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.Булево") Тогда
			ВерныйОтветБулево = ВерныйОтвет;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ФорматированныйТекст = ТекущийОбъект.Формулировка.Получить();
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.Формулировка = Новый ХранилищеЗначения(ФорматированныйТекст, Новый СжатиеДанных(9));
	
	ТекстHTML = "";
	Вложения = Новый Структура;
	ФорматированныйТекст.ПолучитьHTML(ТекстHTML, Вложения);
	
	ТекущийОбъект.ТекстФормулировки = СтроковыеФункцииКлиентСервер.ИзвлечьТекстИзHTML(ТекстHTML);
...
&НаКлиенте
Процедура ВерныйОтветБулевоПриИзменении(Элемент)
	ВерныйОтветПриИзменении(ВерныйОтветБулево);
КонецПроцедуры

&НаКлиенте
Процедура ВерныйОтветДатаПриИзменении(Элемент)
	ВерныйОтветПриИзменении(ВерныйОтветДата);
КонецПроцедуры

&НаКлиенте
Процедура ВерныйОтветСтрокаПриИзменении(Элемент)
	ВерныйОтветПриИзменении(ВерныйОтветСтрока);
КонецПроцедуры

&НаКлиенте
Процедура ВерныйОтветЧислоПриИзменении(Элемент)
	ВерныйОтветПриИзменении(ВерныйОтветЧисло);
КонецПроцедуры
...
&НаКлиенте
Процедура УстановитьВерным(Команда)
	
	ТекущиеДанные =  Элементы.ТаблицаВариантыОтветов.ТекущиеДанные;
	
	Если ТекущиеДанные = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбран какой-либо вариант ответа'"), ,"Элементы.ТаблицаВариантыОтветов");
		Возврат;
	КонецЕсли;
	...
	Если Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.ОдинВариантИз") Тогда
		МассивВерныхВариантовОтветов.Добавить(ТекущиеДанные.Ссылка);
	ИначеЕсли Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.НесколькоВариантовИз") Тогда
		Для Каждого Строка Из Объект.ВерныеОтветы Цикл
			МассивВерныхВариантовОтветов.Добавить(Строка.Ответ);
		КонецЦикла;
		Если ТекущиеДанные.Верный Тогда
			МассивВерныхВариантовОтветов.Удалить(МассивВерныхВариантовОтветов.Найти(ТекущиеДанные.Ссылка));
		Иначе
			МассивВерныхВариантовОтветов.Добавить(ТекущиеДанные.Ссылка);
		КонецЕсли;
	КонецЕсли;
	
	Объект.ВерныеОтветы.Очистить();
	Для каждого Строка из МассивВерныхВариантовОтветов Цикл
		НоваяСтрока = Объект.ВерныеОтветы.Добавить();
		НоваяСтрока.Ответ = Строка;
	КонецЦикла;
...
Процедура УправлениеВидимостью()
	...
	ИначеЕсли Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.Дата") Тогда
		...
		Элементы.ЗависимыеПараметры.ТекущаяСтраница = Элементы.СтраницаДата;
		
	ИначеЕсли Объект.ТипОтвета = ПредопределенноеЗначение("Перечисление.ТипыОтветовНаВопрос.Булево") Тогда
		
		Элементы.ЗависимыеПараметры.ТекущаяСтраница = Элементы.СтраницаБулево;
...
Процедура ОткрытьФормуЭлементаСправочникаВопросыОтветовАнкет(Элемент,РежимДобавления)
	...
	ОткрытьФорму("Справочник.ВариантыОтветовТестов.Форма.ФормаЭлемента",СтруктураПараметров,Элемент);
	...
	Для каждого ЗначениеПеречисления Из Метаданные.Перечисления.ТипыОтветовНаВопрос.ЗначенияПеречисления Цикл
		...
		Если Перечисления.ТипыОтветовНаВопрос[ЗначениеПеречисления.Имя] = Перечисления.ТипыОтветовНаВопрос.ЗначениеИнформационнойБазы Тогда 
			...
			Для каждого ДоступныйТип Из РеквизитФормыВЗначение("Объект").Метаданные().Тип.Типы() Цикл
				...
				Если ДоступныйТип = Тип("Строка") ИЛИ ДоступныйТип = Тип("Булево") ИЛИ ДоступныйТип = Тип("Число") ИЛИ ДоступныйТип = Тип("Дата") ИЛИ ДоступныйТип = Тип("СправочникСсылка.ВариантыОтветовТестов") Тогда
					Продолжить;
				КонецЕсли;
...
&НаКлиенте
Процедура ВерныйОтветПриИзменении(ВерныйОтвет)
	
	Объект.ВерныеОтветы.Очистить();
	НоваяСтрока = Объект.ВерныеОтветы.Добавить();
	НоваяСтрока.Ответ = ВерныйОтвет;
