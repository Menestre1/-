...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	УстановитьЗависимостиДополнительныхРеквизитов(ТекущийОбъект);
...
&НаСервере
Процедура УстановитьЗависимостиДополнительныхРеквизитов(ТекущийОбъект)
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		...
		Элементы.СписокДополнительныхРеквизитов.Заголовок = СтрШаблон(
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Элементы.РежимДобавленияРеквизита.Заголовок = СтрШаблон(
...
Процедура ОбновитьСоставЭлементовФормы(ТекстПредупреждения = "")
	...
	Иначе
		...
		Элементы.ЗадатьУсловиеЗаполнения.Доступность  = Ложь;
		Элементы.ЗадатьУсловиеДоступности.Доступность = Ложь;
		Элементы.ЗадатьУсловиеВидимости.Доступность   = Ложь;
	КонецЕсли;
	...
	Иначе
		...
		Элементы.УточнениеСпискаЗначенийКомментарий.Заголовок =
			СтрШаблон(ШаблонУточнения, СвойстваВладельца.Заголовок, Строка(СвойстваВладельца.НаборСвойств)) + "  ";
	КонецЕсли;
		...
		Если СписокНаборов.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств)
			   И СписокНаборов.Количество() < 2 Тогда
				...
				КонецЕсли;
				ТекстКомментария = СтрШаблон(ШаблонУточнения, СокрЛП(СписокНаборов[0].Представление));
			Иначе
				Если СписокНаборов.Количество() > 1 Тогда
					Если Объект.ЭтоДополнительноеСведение Тогда
					...
					ТекстКомментария = СтрШаблон(ШаблонУточнения, Формат(СписокНаборов.Количество(), "ЧГ="), СтрокаНаборы);
				Иначе
					Если Объект.ЭтоДополнительноеСведение Тогда
...
Процедура ЗаполнитьСписокДействийПриДобавленииРеквизита()
	...
	Иначе
		ШаблонВставки = ".";
	КонецЕсли;
	СоздатьКопированием = СтрШаблон(СоздатьКопированием, ШаблонВставки);
